...
class SEIGrowth(BaseModel):
    ...
    def set_rhs(self, variables):
        ...
        if self.reaction == "SEI on cracks":
            if self.reaction_loc == "x-average":
                roughness = variables[f"X-averaged {domain} {self.phase_name}electrode roughness ratio"]
            else:
                roughness = variables[f"{Domain} {self.phase_name}electrode roughness ratio"]
