"""
ec_reaction_limited_swap_test.py -- degradation_test_matrix_plan.md Section 1a, step 1:
switch the tuned base-case recipe's SEI submodel from "reaction limited" to
"ec reaction limited" (Yang et al. 2017), keeping EVERY other tuned parameter exactly as
validated (SI_MULT, SI_CRIT_STRESS, SI_LAM_PROP_BASELINE, GR_DIV, GR_CRIT_STRESS,
GR_LAM_PROP_BASELINE, crack-rate multipliers, pore buffering at the baseline f0=0.7/
width=0.01), and check how far off the knee timing/LLI/LAM balance lands from the
already-validated "reaction limited" baseline.

No dry-out wrapper involved yet -- this is step 1 in isolation, per the plan ("no dry-out
yet, no pore-buffering-off comparison, just the switch in isolation"). "ec reaction
limited"'s own kinetic parameters (k_sei, D_ec, c_ec_0) are left at si_gr_expansion.py's
existing defaults (4541.0 mol/m3, 2e-18 m2/s, 1e-12 m/s for both phases,
:824-826/:841-843) -- untuned against this recipe's targets, deliberately, since the whole
point of this check is to see how far off those defaults land before deciding whether to
retune them (plan step 2/3).

Runs BOTH SEI options side by side (same recipe, same pore-buffering settings) to 50% SoH,
using the same bug-fixed SoH-termination helper validated in
expansion_pattern_test/physical_f0_width_grid_test.py.
"""
import os
...
# Base-case recipe -- IDENTICAL to test_pore_buffering/pore_buffering_degradation_test_1000cyc.py
# and every expansion_pattern_test baseline point. Held fixed for BOTH SEI options -- this
# test isolates the SEI-submodel swap alone.
SI_MULT = 18.0
LAM_MULT = 1.0
...
def build_param_updates():
    return {
        ...
        # "ec reaction limited"'s own kinetic parameters (k_sei, D_ec, c_ec_0) are
        # DELIBERATELY left at si_gr_expansion.py's existing defaults here (not
        # overridden) -- untuned against this recipe, on purpose, per plan step 1.
    }
...
def current_soh(sol):
    """Reuses the extraction-loop-based SoH check validated in
    expansion_pattern_test/physical_f0_width_grid_test.py -- indexing
    sol.cycles[0]/[-1] directly was tried there and returned cap=0 for every cycle
    (stale/unprocessed step data on a Solution still being extended via
    starting_solution); this loop-based approach is the one that actually works."""
    caps = []
...
def run_case(label, sei_option):
    options = dict(MODEL_OPTIONS_BASE, **{"SEI": sei_option})
    param = pybamm.ParameterValues("si_gr_expansion")
    param.update(build_param_updates(), check_already_exists=False)
    ...
    # crude knee detector: throughput at which SoH first crosses below 90%, as a simple,
    # reproducible proxy -- not the full piecewise-linear-breakpoint method
    # conditions_test_matrix_plan.md proposes (overkill for this quick isolation check).
    ...
    print(f"{tag} final: LLI={LLI_final:.2f}%  LAM_neg={LAM_neg_final:.2f}%  "
          f"final SoH={soh_arr[-1]:.3f}  cycles={total_cycles}  "
          f"knee(SoH<90%) throughput={knee_thr:.1f} A.h  stop_reason={stop_reason}")

    return dict(
        label=label, sei_option=sei_option,
        ...
        knee_thr=knee_thr, total_cycles=total_cycles, stop_reason=stop_reason,
    )


print("\n=== reaction limited (already-validated baseline) ===")
result_reaction = run_case("reaction limited", "reaction limited")

print("\n=== ec reaction limited (untuned defaults, this check's whole question) ===")
result_ec_reaction = run_case("ec reaction limited", "ec reaction limited")

results = [result_reaction, result_ec_reaction]

# ---------------------------------------------------------------------------
# plot
# ---------------------------------------------------------------------------
fig, ax = plt.subplots(2, 2, figsize=(13, 10))
colors = {"reaction limited": "tab:blue", "ec reaction limited": "tab:red"}

for r in results:
    ax[0, 0].plot(r["thr"], r["soh"] * 100, "o-", ms=3, color=colors[r["sei_option"]],
...
for r in results:
    ax[0, 1].plot(r["thr"], r["cell_amplitude"] * 1e6, "o-", ms=3, color=colors[r["sei_option"]],
                  label=r["label"])
...
for r in results:
    ax[1, 0].plot(r["thr"], r["k"], "o-", ms=3, color=colors[r["sei_option"]], label=r["label"])
...
ax[1, 1].set_title("LLI vs. LAM balance at final cycle")
ax[1, 1].legend(fontsize=8)
ax[1, 1].grid(alpha=0.3)

fig.suptitle(
    f"SEI submodel swap check: 'reaction limited' vs. 'ec reaction limited' "
    f"(same recipe otherwise, base-case SI_MULT={SI_MULT:g}, run to {SOH_TARGET:.0%} SoH)"
)
plt.tight_layout()
os.makedirs(os.path.join(SCRIPT_DIR, "pics"), exist_ok=True)
outpath = os.path.join(SCRIPT_DIR, "pics", "ec_reaction_limited_swap_test_result.png")
plt.savefig(outpath, dpi=150)
print(f"\nSaved: {outpath}")

print("\n--- summary ---")
for r in results:
    print(f"{r['label']}: cycles={r['total_cycles']}  final SoH={r['soh'][-1]:.3f}  "
          f"knee(SoH<90%)~{r['knee_thr']:.1f} A.h  LLI={r['LLI_final']:.2f}%  "
          f"LAM_neg={r['LAM_neg_final']:.2f}%  stop_reason={r['stop_reason']}")
